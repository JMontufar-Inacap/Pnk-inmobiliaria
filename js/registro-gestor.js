...
function setError(id, msg) {
  ...
  if (input) input.style.borderColor = msg ? '#EF4444' : '';
}
...
['rut','nombre','fecha_nac','sexo','correo','telefono',
 'password','password2','terminos'].forEach(bindClearOnInput);
...
function doRegistro() {
  ...
  if (!correo) {
    ...
    setError('correo', 'Ingresa un correo electrónico válido (ej: correo@ejemplo.cl).');
    hayError = true;
  }
  ...
  if (!telefono) {
    ...
    setError('telefono', 'Ingresa un número de teléfono válido (ej: +56 9 1234 5678).');
    hayError = true;
  }
  ...
  if (!terminos) {
    setError('terminos', 'Debes aceptar los Términos y Condiciones para continuar.');
    hayError = true;
  }

  if (hayError) {
    const primerError = document.querySelector('.field-error[style*="block"]');
    if (primerError) primerError.scrollIntoView({ behavior: 'smooth', block: 'center' });
    return;
  }

  const res = PNK_DB.registrarUsuario({
    rut, nombre, correo, password, rol: 'gestor',
    sexo, telefono, fecha_nac, num_bienes_raices: null, penka_id: null
  });

  if (!res.ok) {
    if (res.msg && res.msg.toLowerCase().includes('rut')) {
      setError('rut', res.msg);
    } else if (res.msg && res.msg.toLowerCase().includes('correo')) {
  ...
  Swal.fire({
    ...
    html: `
      <p style="margin-bottom:.25rem;">Revisaremos tus antecedentes y te notificaremos pronto.</p>
      <p style="font-size:.8rem;color:#6B7280;margin-top:.5rem;">
        Al ser aprobado recibirás tu <strong>PENKA_ID</strong> y podrás comenzar a generar ingresos adicionales.
      </p>
    `,
